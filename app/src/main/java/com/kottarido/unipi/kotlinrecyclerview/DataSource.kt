...
// auti i klasi dimiourgei ta data (blogs) pou tha provaloume sto recycler view
// se real world apps auta ta data sinithos ta travame apo databases px fb rest api klp
class DataSource {

    // to companion object einai ena singleton object, stou opoiou ta attributes mporoume na exoume
    // prosvasi apefthias meso tou onomatos tis class px (DataSource)
    // episis ena companion object mporei na exei kai onoma kai na kalite kai meso tou onomatos tou
    companion object{
        // dimiourgei mia lista pou tha periexei ta blog posts
        var list = ArrayList<BlogPost>()

        // dimiourgei tin methodo pou prostheti posts stin lista
        // kai dexete os orismata ta stixia tou blog kai episrefi tin list me ta blogs
        fun addPost(title: String, body: String, image: String, username: String): ArrayList<BlogPost>{
            // dimiourgei to bolg
            val blog = BlogPost(title,body,image,username)
            // prothetei to blog stin lista
            list.add(blog)
            return list
        }

        // dimiourgei tin methodo pou ftiaxnei to arxiko dataset
